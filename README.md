...
The runner agent (instances that run the docker machine) can also autoscale based on the value of the `var.runners_max_replicas` set and `var.runners_target_autoscale_cpu_utilization`.

### GitLab CI docker runner

In this scenario _not_ docker machine is used but docker to schedule the builds. Builds will run on the same compute instance as the agent. Auto scaling is supported via the `var.runners_max_replicas` and `var.runners_target_autoscale_cpu_utilization`.
