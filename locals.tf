locals {
  ...
  docker_machine_labels = join(",", [for key, value in merge({ "managed-by" = "gitlab-runner", "runner-executor" : "docker-machine" }, var.labels) : "${key}:${value}"])
  ...
  template_startup_script = templatefile("${path.module}/templates/startup-script.sh.tpl",
    {
      ...
      giltab_runner_description             = lookup(var.gitlab_runner_registration_config, "description", "")
  ...
  template_runner_config = templatefile("${path.module}/templates/runner-config.toml.tpl",
    {
      ...
      runners_machine_type           = var.docker_machine_machine_type
      runners_disk_type              = var.docker_machine_disk_type
      runners_disk_size              = var.docker_machine_disk_size
      runners_tags                   = join(",", distinct(concat(["gitlab"], var.docker_machine_tags)))
      runners_labels                 = local.docker_machine_labels
      runners_use_internal_ip        = var.docker_machine_use_internal_ip
